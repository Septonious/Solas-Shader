...
float RingLensTransform(float lensFlare) {
	return pow4(1.0 - pow8(1.0 - pow(lensFlare, 0.25)));
}
...
vec3 LensTint(vec3 lens, float truePos) {
	...
	lens = mix(lens, GetLuminance(lens) * vec3(0.25, 0.45, 1.00), isMoon * 0.98);
...
void LensFlare(inout vec3 color, vec2 lightPos, float truePos, float multiplier) {
	...
	float falloffIn = pow2(clamp(falloffBase * 10.0, 0.0, 1.0));
