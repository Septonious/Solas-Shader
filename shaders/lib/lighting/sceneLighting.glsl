...
void getSceneLighting(inout vec3 albedo, in vec3 screenPos, in vec3 viewPos, in vec3 worldPos, in vec3 normal, in vec2 lightmap,
    ...
    float giVisibility = clamp(length(fullShadow * rainFactor * sunVisibility), 0.0, 1.0) * int(emission == 0.0 && specular == 0.0);

    if (giVisibility != 0.0) {
        coloredLightingIntensity += mix(0.0, GLOBAL_ILLUMINATION_STRENGTH, giVisibility);
    }
    #endif
