...
void getSceneLighting(inout vec3 albedo, in vec3 viewPos, in vec3 worldPos, in vec3 normal, in vec2 lightmap, in float emission, in float subsurface, in float foliage, in float specular) {
    ...
    #else
    float blockLightMap = min(pow12(lightmap.x) * 1.25 + pow3(lightmap.x) * 0.75, 1.0) * float(emission == 0.0);
    #endif
    ...
    vec3 bloom = texture2D(colortex7, gl_FragCoord.xy / vec2(viewWidth, viewHeight)).rgb;
         ...
         bloom = clamp(1.5 * bloom * pow(getLuminance(bloom) + 0.005, -0.75), 0.0, 1.0);
         bloom *= (0.2 + blockLightMap * 0.8) * BLOOM_STRENGTH;
