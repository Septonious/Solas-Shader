	...
	if (visibility > 0.1) {
		...
		for (int i = 0; i < samples; i++) {
			...
			if (auroraDistanceFactor > 0.0) {
				...
				float blobNoise = max(0.0, texture2D(noisetex, coordDeform * 0.0075 + frameTimeCounter * 0.0001).b - 0.2 - kpIndex * 0.3);
				...
				baseNoise = max(1.0 - 2.0 * abs(baseNoise - 3.0) - (1.0 - kpIndex * 0.75) * 0.5, 0.0);
				baseNoise *= baseNoise;
				...
				float colorMixer = clamp(texture2D(noisetex, coord * 0.00125).b * kpIndex * kpIndex * (1.0 + pow4(kpIndex) * 3.0), 0.0, 1.0);

				vec3 auroraColor1 = mix(vec3(0.6, 4.2, 0.2), vec3(3.4, 0.1, 1.5), pow(currentStep, 0.25));
