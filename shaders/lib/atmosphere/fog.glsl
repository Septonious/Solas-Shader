...
void getNormalFog(inout vec3 color, in vec3 atmosphereColor, in vec3 viewPos, in vec3 worldPos, in float lViewPos, in float lWorldPos, in float z0) {
	...
	float altitudeFactor = FOG_HEIGHT + noise * 10.0 + timeBrightness * 35.0 - isJungle * 15.0;
	...
	float density = FOG_DENSITY * (1.0 + (sunVisibility - timeBrightness) * 0.5 + moonVisibility) * (0.5 + noise);
	...
	vec3 fogCol = fmix(caveMinLightCol * (1.0 - isCaveBiome) + caveBiomeColor,
                   fmix(pow(atmosphereColor, vec3(1.0 - sunVisibility * 0.5)) * 0.75, nSkyColor, sunVisibility * min((1.0 - wetness) * (1.0 - fog) + 0.4 * timeBrightnessSqrt, 1.0)),
                   caveFactor);
