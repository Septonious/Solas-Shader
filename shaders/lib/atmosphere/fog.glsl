void getNormalFog(inout vec3 color, vec3 viewPos, in vec3 worldPos, in vec3 atmosphereColor) {
	...
	#ifdef OVERWORLD
	//Variables
	float fogAltitude = clamp(pow16((worldPos.y + cameraPosition.y + 1000.0 - FOG_HEIGHT) * 0.001), 0.0, 1.0 - rainStrength * 0.25);
	...
	vec3 fogColor = mix(atmosphereColor, skyColor * skyColor, (0.6 - timeBrightness * 0.5) * sunVisibility * (1.0 - rainStrength)) * fog;
