void getSunMoon(inout vec3 color, in vec3 nViewPos, in vec3 lightSun, in vec3 lightNight, in float VoS, in float VoM, in float VoU, in float caveFactor) {
	float visibility = clamp(VoU, 0.0, 1.0) * (1.0 - rainStrength) * caveFactor;
	...
	if (visibility > 0.0) {
		...
		if (moon > 0.0 && moonPhase > 0) { // Moon phases, uses the same method as Complementary v4
			...
			moon *= clamp(1.0 - pow8(pow32(dot(nViewPos, newSunVec))), 0.0, 1.0);
		}
		
		vec3 sunMoonColor = mix(moon * normalize(lightNight), sun * normalize(lightSun), sunVisibility) * 8.0;
			 sunMoonColor *= clamp(pow6(length(sunMoonColor)), 0.0, 1.0);
