...
void computeVolumetricClouds(inout vec4 vc, in vec3 atmosphereColor, float z1, float dither, inout float currentDepth) {
	...
	if (visibility > 0.0) {
		...
		if (maxDist >= 0.0 && sampleCount > 0) {
			...
			vec3 cloudAmbientColor = mix(ambientCol, ambientCol * skyColor, sunVisibility * sunVisibility) * (0.35 + sunVisibility * sunVisibility * 0.3);
			vec3 cloudLightColor = mix(lightCol, skyColor, 0.15 * sunVisibility * sunVisibility) * (1.0 + scattering * 2.0);
