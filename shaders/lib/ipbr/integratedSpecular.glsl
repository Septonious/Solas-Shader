...
void getIntegratedSpecular(in vec4 albedo, in vec3 normal, in vec2 worldPos, in vec2 lightmap, inout float specular) {
    ...
    float NoU = clamp(dot(normal, upVec), 0.0, 0.65);
