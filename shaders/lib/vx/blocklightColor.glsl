const vec3[] blocklightColorArray = vec3[](
	...
	vec3(1.00, 0.50, 0.15) * 3.0,
	//Soul Torch, Soul Lantern, Soul Campfire, Soul Fire
	vec3(0.10, 0.60, 1.00) * 2.5,
	//End Rod
	vec3(1.00, 0.50, 0.90) * 2.5,
	//Sea Lantern
	vec3(0.70, 0.90, 1.00) * 3.5,
	//Glowstone
	vec3(1.00, 0.65, 0.25) * 3.5,
    ...
    //Orchre Froglight
    normalize(vec3(1.00, 0.55, 0.25)) * 3.5,
    //Verdant Froglight
    normalize(vec3(0.0, 1.00, 0.05)) * 3.5,
    //Pearlescent Froglight
    normalize(vec3(1.00, 0.20, 0.90)) * 3.5,
	#ifdef EMISSIVE_FLOWERS
    //Red flowers
    normalize(vec3(1.00, 0.05, 0.05)) * 0.35,
    //Pink flowers
    normalize(vec3(0.80, 0.20, 0.60)) * 0.35,
    //Yellow flowers
    normalize(vec3(0.80, 0.50, 0.05)) * 0.35,
    //Blue flowers
    normalize(vec3(0.00, 0.15, 1.00)) * 0.35,
    //White flowers
    normalize(vec3(0.80, 0.80, 0.80)) * 0.35,
    //Orange flowers
    normalize(vec3(1.00, 0.70, 0.05)) * 0.35,
	#else
	vec3(0.0),
	vec3(0.0),
	vec3(0.0),
    ...
    //Beacon
    vec3(0.6, 0.7, 1.0) * 3.0,
	//Sculks, non-emissive
	vec3(0.0),
	//Sculk Sensor
	vec3(0.20, 0.55, 1.00) * 2.0,
	//Calibrated Sculk Sensor
	vec3(1.00, 0.25, 0.75) * 2.0,
	//Fungi
	vec3(0.0),
	//Crimson Stem & Hyphae
