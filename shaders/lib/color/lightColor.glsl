...
vec3 lightColRaw = mix(lightNight, mix(vec3(lightSun.r, lightSun.g, lightSun.b * (2.0 - clamp((mefade + timeBrightness) * 8.0, 0.0, 1.0))), normalize(skyColor + 0.0001), 0.1), sunVisibility * sunVisibility);
#else
...
float ambientIntensity = mix(AMBIENTINTENSITY_N, mix(AMBIENTINTENSITY_D * 0.6, AMBIENTINTENSITY_D, timeBrightness), sunVisibility * sunVisibility);
