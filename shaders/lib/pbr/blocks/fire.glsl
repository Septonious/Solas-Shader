    ...
    albedo.rgb *= 0.5;
    albedo.rgb *= pow(vec3(TLCF_R, TLCF_G, TLCF_B), vec3(1.0 - 0.25 * lAlbedo * lAlbedo)) * pow4(fireNoise) * 28.0;
    ...
    albedo.rgb *= 0.5;
    albedo.rgb *= pow(vec3(SOUL_R, SOUL_G, SOUL_B), vec3(1.0 - 0.25 * lAlbedo * lAlbedo)) * pow4(fireNoise) * 28.0;
    albedo.rgb = clamp(albedo.rgb, 0.0, 1.0);
    emission = length(albedo.rgb); 
}