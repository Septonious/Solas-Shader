	...
	isOreEmissive += float(material == 22);
	#endif

	#ifdef EMISSIVE_DIAMOND_ORE
	isOreEmissive += float(material == 23);
	#endif

	#ifdef EMISSIVE_COPPER_ORE
	isOreEmissive += float(material == 24);
	#endif

	#ifdef EMISSIVE_LAPIS_ORE
	isOreEmissive += float(material == 25);
	#endif

	#ifdef EMISSIVE_GOLD_ORE
	isOreEmissive += float(material == 26);
	#endif

	#ifdef EMISSIVE_IRON_ORE
	isOreEmissive += float(material == 27);
	#endif

	#ifdef EMISSIVE_REDSTONE_ORE
	isOreEmissive += float(material == 28);
	#endif
