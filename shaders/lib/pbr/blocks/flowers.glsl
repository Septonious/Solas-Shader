	...
	float emissionFactor = 0.175 * lAlbedo * (0.25 + (1.0 - clamp(length(viewPos) * 0.25, 0.0, 1.0)));
	float noise = texture(noisetex, (worldPos.xz + cameraPosition.xz + frameCounter * 0.01) * 0.001).b;
