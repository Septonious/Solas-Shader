	...
	if (albedo.r + albedo.g + albedo.b < 2.9) {
		if (albedo.b > albedo.g || albedo.r > albedo.g) {
			emission = emissionFactor * float(albedo.r > 0.6 && albedo.b < 0.3);
			#ifdef OVERWORLD
			emission *= mix(1.0, 0.0, wetness);
			#endif
		}
	...
	if (albedo.r + albedo.g + albedo.b < 2.9) {
		if (albedo.b > albedo.g || albedo.r > albedo.g) {
			emission = emissionFactor * (float(lAlbedo > 0.63) * (float(albedo.r + albedo.g * 0.5 + albedo.b * 0.5 > 0.9) + float(albedo.r - albedo.g - albedo.b > 0.3)));
			#ifdef OVERWORLD
			emission *= mix(1.0, 0.0, wetness);
			#endif
		}
